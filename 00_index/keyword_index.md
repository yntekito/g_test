...
- **分散型強化学習（Distributed Reinforcement Learning）**: 複数ワーカーを並列活用し学習を高速化、経験共有、A3C・Ape-X・IMPALA等 → [05_machine_learning/reinforcement_learning.md](../05_machine_learning/reinforcement_learning.md)
- **A3C（Asynchronous Advantage Actor-Critic）**: 非同期分散強化学習の代表手法、ワーカーが独立に大域パラメータ更新 → [05_machine_learning/reinforcement_learning.md](../05_machine_learning/reinforcement_learning.md)
- **Ape-X（Distributed Prioritized Experience Replay）**: 分散データ収集・集中学習、優先度付き経験再生 → [05_machine_learning/reinforcement_learning.md](../05_machine_learning/reinforcement_learning.md)
- **IMPALA（Importance Weighted Actor-Learner Architecture）**: 超大規模並列分散RL、数百～数千ワーカー、V-trace補正 → [05_machine_learning/reinforcement_learning.md](../05_machine_learning/reinforcement_learning.md)
- **ワーカー（Worker）**: 分散RLで環境と相互作用し経験を収集する並列実行エージェント → [05_machine_learning/reinforcement_learning.md](../05_machine_learning/reinforcement_learning.md)
...
- **データリーケージ（Data Leakage）**: 本来使えない情報が訓練データに混入し過度に楽観的な性能評価になる問題、実運用で性能が劇的に低下 → [05_machine_learning/overfitting_underfitting.md](../05_machine_learning/overfitting_underfitting.md)
- **ターゲットリーケージ（Target Leakage）**: 予測時に入手不可能な情報が特徴量に含まれる問題 → [05_machine_learning/overfitting_underfitting.md](../05_machine_learning/overfitting_underfitting.md)
- **トレーニング-テストコンタミネーション（Train-Test Contamination）**: テストデータ情報が訓練データに漏れる問題 → [05_machine_learning/overfitting_underfitting.md](../05_machine_learning/overfitting_underfitting.md)
- **時間的リーケージ（Temporal Leakage）**: 時系列データで未来の情報が過去のデータに混入する問題 → [05_machine_learning/overfitting_underfitting.md](../05_machine_learning/overfitting_underfitting.md)
...
- **Batch Normalization（バッチ正規化）**: CNNの隠れ層でミニバッチごとに各チャンネルを正規化、学習安定化・高速化を実現 → [06_deep_learning/cnn.md](../06_deep_learning/cnn.md)
- **内部共変量シフト（Internal Covariate Shift）**: 学習中に各層の入力分布が変化する現象、Batch Normalizationで抑制 → [06_deep_learning/cnn.md](../06_deep_learning/cnn.md)
- **Layer Normalization**: サンプルごとに全特徴を正規化、RNN・Transformer向け → [06_deep_learning/cnn.md](../06_deep_learning/cnn.md)
- **Instance Normalization**: サンプル×空間で正規化、スタイル変換向け → [06_deep_learning/cnn.md](../06_deep_learning/cnn.md)
- **Group Normalization**: チャンネルをグループ分けして正規化、小バッチ対応 → [06_deep_learning/cnn.md](../06_deep_learning/cnn.md)
...
- **スペクトル包絡**: 音声スペクトルの大まかな形状、音色の特徴を表現、声道の共振特性を反映 → [07_ai_applications/speech_processing.md](../07_ai_applications/speech_processing.md)
- **音色（timbre）**: 音の質的な特徴、スペクトル包絡の違いで認識される、同じピッチでも楽器や母音で異なる → [07_ai_applications/speech_processing.md](../07_ai_applications/speech_processing.md)
- **ピッチ（pitch）**: 音の高さ、基本周波数で決まる、声帯振動の周期 → [07_ai_applications/speech_processing.md](../07_ai_applications/speech_processing.md)
- **基本周波数（F0）**: ピッチを決定する最低周波数成分、声帯の振動周期 → [07_ai_applications/speech_processing.md](../07_ai_applications/speech_processing.md)
- **フォルマント（Formant）**: 声道の共振周波数、母音の識別に重要、スペクトル包絡のピーク → [07_ai_applications/speech_processing.md](../07_ai_applications/speech_processing.md)
...
- **測定誤差（Measurement Error）**: データ収集・測定時に生じる誤差の総称 → [10_math_statistics/statistics.md](../10_math_statistics/statistics.md)
- **偶然誤差（Random Error）**: ランダムに発生する予測不可能な誤差、測定回数増加で減少 → [10_math_statistics/statistics.md](../10_math_statistics/statistics.md)
- **系統誤差（Systematic Error / Bias）**: 一定方向に偏る再現性のある誤差、補正で除去可能 → [10_math_statistics/statistics.md](../10_math_statistics/statistics.md)
- **訓練誤差（Training Error）**: 訓練データに対するモデルの予測誤差、損失関数の値 → [10_math_statistics/statistics.md](../10_math_statistics/statistics.md)
- **テスト誤差（Test Error）**: テストデータでの予測誤差、汎化性能の指標 → [10_math_statistics/statistics.md](../10_math_statistics/statistics.md)
- **バイアス（機械学習）**: モデルの表現力不足による系統的な誤差 → [10_math_statistics/statistics.md](../10_math_statistics/statistics.md)
- **バリアンス（Variance）**: 訓練データへの過剰適合による誤差のばらつき → [10_math_statistics/statistics.md](../10_math_statistics/statistics.md)
- **バイアス-バリアンストレードオフ**: モデル複雑さの最適化、汎化誤差=バイアス²+バリアンス+ノイズ → [10_math_statistics/statistics.md](../10_math_statistics/statistics.md)
