...
| **AdaGrad** | 各パラメータごとに適応 | 自動（単調減少） | スパースデータ |
| **RMSprop** | AdaGradの改良（減衰平均） | 自動 | RNN学習 |
...
**定義**:  
**各パラメータごとに学習率を適応的に調整する最適化アルゴリズム**。過去の勾配の二乗和を累積し、頻繁に更新されるパラメータほど学習率を小さく、まれにしか更新されないパラメータほど学習率を大きく保つ。スパースなデータ（自然言語処理の単語埋め込みなど）で特に有効。

**更新式**:
$$G_t = G_{t-1} + (\nabla L(\theta_t))^2$$
$$\theta_{t+1} = \theta_t - \frac{\eta}{\sqrt{G_t + \epsilon}} \nabla L(\theta_t)$$

- $G_t$: **過去の勾配の二乗和を累積**（パラメータごとに保持）
- $\eta$: 初期学習率（例: 0.01）
- $\epsilon$: ゼロ除算を防ぐ小さな値（例: $10^{-8}$）

**動作原理**:
1. **頻繁に更新されるパラメータ**: $G_t$ が大きくなる → $\frac{\eta}{\sqrt{G_t}}$ が小さくなる → 学習率が**減少**
2. **まれに更新されるパラメータ**: $G_t$ が小さい → $\frac{\eta}{\sqrt{G_t}}$ が大きい → 学習率が**維持**
3. **時間経過とともに**: すべてのパラメータで $G_t$ は**単調増加** → 学習率は**単調減少**

**利点**:
- ✅ 学習率の手動調整が不要（自動適応）
- ✅ スパースなデータで高性能（単語埋め込み、推薦システムなど）
- ✅ 各パラメータごとに最適な学習率

**欠点**:
- ❌ **学習率が単調減少し続ける**（$G_t$ は累積のみで減少しない）
- ❌ 長時間学習すると学習率がほぼゼロになり**学習が停止**
- ❌ 深層学習の長期学習には不向き

**典型的な使用例**:
- 自然言語処理の単語埋め込み（Word2Vec、GloVeなど）
- 推薦システムのスパースな特徴量
- オンライン学習

**改良版**:
- **RMSprop**: $G_t$ を**指数移動平均**に変更し、古い勾配情報を忘却 → 学習率の単調減少問題を解決
- **Adam**: RMSpropに**モメンタム**を追加 → 現在最も広く使われる
...
**定義**:  
AdaGradの**学習率が単調減少する問題を解決**するため、過去の勾配を**指数移動平均**で累積する。古い勾配情報を徐々に忘却し、最近の勾配情報を重視する。
...
**問題**: AdaGradの説明として、最も適切な選択肢を1つ選べ。

1. ✅ **過去の勾配の二乗和を累積し、各パラメータごとに学習率を適応的に調整する**  
   → **正解**。AdaGradの核心的特徴。$G_t = G_{t-1} + (\nabla L)^2$ で累積し、$\frac{\eta}{\sqrt{G_t}}$ で学習率を調整。

2. ❌ 過去の勾配を指数移動平均で計算し、最近の勾配情報を重視する  
   → **不正解**。これは**RMSprop**の説明。AdaGradは累積のみで忘却しない。

3. ❌ モメンタムと適応的学習率を組み合わせ、1次と2次モーメントを利用する  
   → **不正解**。これは**Adam**の説明。AdaGradはモメンタムを持たない。

4. ❌ 学習率を一定に保ち、勾配の方向のみを更新する  
   → **不正解**。これは通常の**SGD**。AdaGradは学習率を適応的に変える。

**ひっかけポイント**:
- **AdaGrad vs RMSprop**: 「累積」か「移動平均」か（AdaGrad=累積、RMSprop=移動平均）
- **AdaGrad vs Adam**: 「モメンタムの有無」（AdaGrad=なし、Adam=あり）
- **単調減少問題**: AdaGradの主な欠点は「学習率が単調に減少し続ける」こと
- **用途**: AdaGradは「スパースデータ」に強い（自然言語処理、推薦システム）

**比較のポイント（頻出）**:

| 手法 | 学習率調整 | 勾配の累積方法 | モメンタム | 主な欠点 |
|------|-----------|--------------|-----------|---------|
| **AdaGrad** | 各パラメータごとに適応 | **二乗和を累積** | なし | 学習率が**単調減少** |
| **RMSprop** | 各パラメータごとに適応 | **指数移動平均** | なし | - |
| **Adam** | 各パラメータごとに適応 | **指数移動平均** | **あり** | 一部タスクで汎化性能劣る |
