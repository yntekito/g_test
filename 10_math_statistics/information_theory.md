...
**問題例**: 「（　）は、2つの確率分布の差異を表す尺度であると同時に、誤差関数としても用いられる。」
...
**答え**: **3. 交差エントロピー（Cross Entropy）**

**理由**:
1. **「2つの確率分布の差異」がキーワード**:
   - 交差エントロピー $H(P, Q) = -\sum_x p(x) \log q(x)$ は、真の分布 $P$ と予測分布 $Q$ の違いを測る
   - $P$ と $Q$ が一致するほど値が小さくなる

2. **「誤差関数（損失関数）としても用いられる」がキーワード**:
   - 機械学習の分類問題で標準的に使用される損失関数
   - ニューラルネットワークの出力層（Softmax）と組み合わせて使用
   - 真のラベル（ワンホットベクトル）と予測確率の差を最小化

**各選択肢の検証**:

| 選択肢 | 定義 | 2つの分布の差異？ | 誤差関数？ | 判定 |
|--------|------|-----------------|----------|------|
| **情報エントロピー** | $H(X) = -\sum p(x) \log p(x)$ | ❌ 1つの分布の不確実性 | ❌ 単独では使わない | ✗ |
| **結合エントロピー** | $H(X,Y) = -\sum p(x,y) \log p(x,y)$ | ❌ 2変数の同時分布 | ❌ 使わない | ✗ |
| **交差エントロピー** | $H(P,Q) = -\sum p(x) \log q(x)$ | ✅ 2つの分布の差異 | ✅ 分類の損失関数 | ✅ |
| **可逆性エントロピー** | - | ❌ 存在しない用語 | ❌ 存在しない | ✗ |

#### 交差エントロピーの詳細（G検定重要ポイント）

**1. 数式の意味**:
$$
H(P, Q) = -\sum_{i} p_i \log q_i
$$
- $p_i$: 真の分布（正解ラベル、ワンホットベクトル）
- $q_i$: 予測分布（モデルの出力確率）
- 真の分布 $P$ で重み付けし、予測分布 $Q$ の対数を計算
...
交差エントロピー損失:
H = -(0×log(0.1) + 1×log(0.7) + 0×log(0.2))
  = -log(0.7) ≈ 0.357
```
...
$$
- $H(P)$: 真の分布のエントロピー（定数）
- $D_{KL}(P||Q)$: KLダイバージェンス（分布の差異）
- **交差エントロピーを最小化 = KLダイバージェンスを最小化**
- 学習では $P$ は固定なので、$H(P, Q)$ を最小化すれば $Q \to P$ に近づく

**4. 他の用語との違い**:

| 用語 | 対象 | 用途 | 試験での問われ方 |
|------|------|------|----------------|
| **エントロピー** | 1つの分布 | 不確実性の測定 | 「情報の不確実性」 |
| **結合エントロピー** | 2変数の同時分布 | 2変数の同時不確実性 | 「H(X,Y)=H(X)+H(Y\|X)」 |
| **交差エントロピー** | 2つの分布の差異 | **分類の損失関数** | **「誤差関数として使用」** |
| **KLダイバージェンス** | 2つの分布の差異 | 分布の距離 | 「非対称な尺度」 |
| **相互情報量** | 2変数の依存性 | 特徴選択 | **「I=0で独立」** |
...
問題文を読む
    ↓
「2つの確率分布の差異」とある？
    ↓ YES → 交差エントロピー or KLダイバージェンス or 相互情報量
    ↓
「誤差関数」「損失関数」とある？
    ↓ YES → 交差エントロピー（正解）
    ↓
...
**6. 混同しやすい選択肢の罠**:
- ❌ 「情報エントロピー」: 1つの分布のみ、2つの分布の差異ではない
- ❌ 「結合エントロピー」: 2変数の同時分布、誤差関数ではない
- ❌ 「可逆性エントロピー」: 存在しない造語（ひっかけ）
- ✅ **「交差エントロピー」: 唯一、両方の条件を満たす**
