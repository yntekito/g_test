...
**定義**：訓練データを人工的に増やして過学習を防ぐ手法。既存データに変換を加えて新しいサンプルを生成し、モデルの汎化性能を向上させる。

**基本的なデータ拡張手法**：
- **画像**：回転、反転、クロッピング、色調変換、アフィン変換
- **テキスト**：同義語置換、バックトランスレーション、文の削除・追加
- **音声**：ノイズ追加、ピッチ変換、時間伸縮
...
**定義**：
**2つの訓練サンプルを線形補間して新しい訓練サンプルを生成するデータ拡張手法**。画像とラベルの両方を混ぜ合わせることで、より頑健なモデルを学習する。

**Mixupの核心的特徴**：
1. **画像の線形補間**：2つの画像をピクセルレベルで線形結合
2. **ラベルの線形補間**：ワンホットラベルも同じ比率で混合
3. **決定境界の滑らかさ向上**：クラス間の遷移を滑らかに学習
...
- $x_i, x_j$：2つの訓練画像
- $y_i, y_j$：2つのラベル（ワンホットベクトル）
- $\lambda \sim \text{Beta}(\alpha, \alpha)$：混合比率（通常 $\alpha = 0.2 \sim 1.0$）
- $\tilde{x}, \tilde{y}$：生成された新しい訓練サンプル
...
- ❌ 混合画像が不自然（人間には理解しづらい）
- ❌ クラス境界が曖昧になる（一長一短）
- ❌ ラベルノイズに弱い（誤ったラベルも混合される）

**試験での問われ方**：

**問題**: Mixupの説明として、**最も適切な選択肢**を1つ選べ。

1. ✅ **2つの訓練サンプルを線形補間し、画像とラベルの両方を混ぜ合わせて新しいサンプルを生成する**  
   → **正解**。Mixupの核心的特徴。$\tilde{x} = \lambda x_i + (1-\lambda) x_j$ で画像を混合し、ラベルも同様に混合。

2. ❌ 画像の一部を切り取って他の画像で置き換え、ラベルは面積比で混合する  
   → **不適切**。これは**CutMix**の説明。Mixupは線形補間、CutMixは領域置換。

3. ❌ 画像をランダムに回転・反転・クロッピングして訓練データを増やす  
   → **不適切**。これは**従来のデータ拡張**。Mixupは複数サンプルを混合する点が異なる。

4. ❌ GANを用いて新しい訓練サンプルを生成し、データ分布を拡張する  
   → **不適切**。これは**GAN**によるデータ生成。Mixupは既存サンプルの線形補間。

**ひっかけポイント**：

| 手法 | 混合方法 | ラベル処理 | 画像の見た目 |
|------|---------|-----------|------------|
| **Mixup** | **線形補間**（全体を混合） | **線形補間**（比率で混合） | 全体が半透明に混ざる |
| **CutMix** | **領域置換**（矩形領域を切り貼り） | **面積比で混合** | 明確な境界で分割 |
| **従来の拡張** | 変換のみ（混合なし） | 変換なし | 元画像の変形のみ |
| **GAN生成** | 新規生成 | 生成クラスのみ | 完全に新しい画像 |

**比較のポイント（頻出）**：

**Mixup vs CutMix**：
- **Mixup**: 全体を線形補間 → 画像が半透明に重なる
- **CutMix**: 矩形領域を切り貼り → 明確な境界で分割

**Mixup vs 従来のデータ拡張**：
- **Mixup**: 複数サンプルを**混合**してラベルも補間
- **従来**: 1サンプルを**変換**するのみ（ラベル不変）

**Mixup vs GAN**：
- **Mixup**: 既存サンプルの**線形補間**（単純・高速）
- **GAN**: 新規サンプルの**生成**（複雑・低速）

##### CutMix（カットミックス）

**定義**：
画像の一部を矩形領域で切り取り、別の画像の対応領域で置き換える。ラベルは面積比で混合する。

**特徴**：
- Mixupと異なり、画像の局所的な特徴を保持
- 物体検出タスクで有効（明確な境界が残る）
- ラベルの混合は切り取った領域の面積比
...
- $\mathbf{M}$：マスク（矩形領域のみ1、それ以外0）
- $\lambda = \frac{|\text{切り取り領域}|}{|\text{画像全体}|}$：面積比

##### AutoAugment（自動データ拡張）

**定義**：
強化学習を用いてデータセットごとに最適なデータ拡張方針を自動探索する手法。

**特徴**：
- 複数の拡張操作を組み合わせて最適な方針を学習
- データセット固有の最適化（ImageNet、CIFAR等で異なる方針）
- 計算コストは高いが、性能向上が大きい
