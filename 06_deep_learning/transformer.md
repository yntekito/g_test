...
### Vision Transformer（ViT）の画像入力処理

**背景**：
- 画像は2次元の空間構造を持つため、そのままTransformerに入力できない
- Transformerは1次元の系列データを前提とする
- 解決策：画像を**パッチ（小領域）に分割**し、各パッチを系列の1要素として扱う
...
[入力画像: 224×224×3 (RGB)]
         ↓
[パッチ分割: 16×16ピクセル単位]
         ↓ (224/16)×(224/16) = 14×14 = 196個のパッチ
[各パッチを平坦化: 16×16×3 = 768次元ベクトル]
         ↓
[線形埋め込み: 768次元 → D次元（例: 768次元）]
         ↓
[位置エンコーディング追加（パッチの空間位置）]
         ↓
[[CLS]トークン + 196個のパッチ埋め込み]
         ↓
[Transformer Encoder（標準的なTransformer処理）]
         ↓
[[CLS]トークンの出力で画像分類]
```
...
**具体例（ImageNet分類）**：
- 入力画像：224×224×3ピクセル
- パッチサイズ：16×16（ViT-B/16の場合）
- パッチ数：(224÷16)² = 14² = **196個**
- 各パッチの次元：16×16×3 = **768次元**
- 系列長：196（パッチ）+ 1（[CLS]トークン）= **197トークン**

**重要ポイント**：
1. **パッチ分割**：畳み込み層を使わず、固定サイズ（16×16等）に機械的に分割
2. **平坦化**：各パッチを2次元→1次元ベクトルに変換（16×16×3 → 768次元）
3. **位置エンコーディング**：パッチの位置（どこにあったか）を学習可能な埋め込みで追加
4. **[CLS]トークン**：BERTと同様、系列先頭に特殊トークンを追加し、その出力で分類
5. **CNNとの違い**：畳み込み層・プーリング層を**使用しない**（純粋なAttentionのみ）

**試験で問われる誤答パターン**：
- ❌ 「畳み込み層で特徴抽出してからTransformerに入力」→ Hybrid ViTの手法（純粋ViTは不使用）
- ❌ 「ピクセル単位で系列化」→ 系列が長すぎて計算不可（50,176個 vs 196個）
- ❌ 「再帰型ネットワークで処理」→ TransformerはRNN不使用
- ❌ 「画像全体を1つのベクトルに圧縮」→ 空間構造が失われる
...
### Vision Transformerの入力処理（試験頻出）

**問：Vision Transformerで画像を入力する際の処理として最も適切なものは？**

✅ **正解パターン**：
- 「画像を固定サイズのパッチ（例：16×16）に分割し、各パッチを平坦化して系列データとして扱う」
- 「パッチごとに1次元ベクトルに変換し、位置エンコーディングを追加してTransformerに入力する」

❌ **誤答パターン**：
- 「畳み込み層で特徴マップを抽出する」→ Hybrid ViTまたはCNNの手法
- 「ピクセル単位で系列化」→ 計算量が膨大（系列長が数万）
- 「再帰型ネットワークで順次処理」→ RNNの手法（Transformerは並列処理）
- 「画像全体を1ベクトルに圧縮」→ 空間構造の損失

**キーワード判定**：
- 正解候補：「パッチ分割」「平坦化」「系列データ」「位置エンコーディング」
- 誤答候補：「畳み込み」「プーリング」「再帰」「ピクセル単位」
