...
## 連鎖律（Chain Rule）の活用場面（★試験重要）

### 定義
**連鎖律（Chain Rule）**は、合成関数の微分を計算するための数学的法則。深層学習では、**誤差逆伝播法（Backpropagation）で各層の重みに対する損失関数の勾配を効率的に計算する**際に活用されます。
...
### 連鎖律の活用が期待される場面（★試験最頻出）

#### 典型的な質問形式
> 「連鎖律（Chain Rule）の活用が期待される場面の説明として、最も適切な選択肢を1つ選べ。」

✅ **正解の選択肢**：
- **多層ニューラルネットワークにおいて、出力層の誤差から各層の重みの勾配を計算する場面**
- **誤差逆伝播法（Backpropagation）で損失関数の各パラメータに対する偏微分を求める場面**
- **深層学習モデルの学習で、各層の重みをどの方向にどれだけ更新すべきかを決定する場面**
- **合成関数の微分を効率的に計算し、勾配降下法で重みを更新する場面**

❌ **不適切な選択肢（混同注意）**：
- データの前処理や正規化 → 連鎖律は不要
- 初期重みの設定 → 連鎖律は不要
- 活性化関数の選択 → 連鎖律は活性化関数の微分に使うが選択自体には不要
- モデルの評価指標の計算 → 連鎖律は不要（誤差計算は微分不要）
- 学習データのシャッフル → 連鎖律は不要

### ニューラルネットワークでの連鎖律の具体例

#### 3層ニューラルネットワーク

```
入力層 → 隠れ層 → 出力層 → 損失
x      → h = σ(w1·x) → y = σ(w2·h) → L = (y - t)²
```

**目標**: 損失 $L$ の重み $w_1$ に対する勾配 $\frac{\partial L}{\partial w_1}$ を計算
...
各項を順に計算：
1. $\frac{\partial L}{\partial y} = 2(y - t)$（損失の微分）
2. $\frac{\partial y}{\partial h} = \sigma'(w_2 \cdot h) \cdot w_2$（活性化関数の微分）
3. $\frac{\partial h}{\partial w_1} = \sigma'(w_1 \cdot x) \cdot x$（活性化関数の微分）

**連鎖律で繋げる**：
$$\frac{\partial L}{\partial w_1} = 2(y - t) \cdot \sigma'(w_2 \cdot h) \cdot w_2 \cdot \sigma'(w_1 \cdot x) \cdot x$$

### 連鎖律なしでは何が困るのか

**問題1: 計算量の爆発**
- 各重みについて独立に微分を計算すると、層数に対して指数的に計算量が増加
- 連鎖律により、一度の逆伝播で全ての重みの勾配を計算可能

**問題2: 実装の複雑さ**
- 層が深くなるほど、直接微分を計算するのは困難
- 連鎖律により、各層の局所的な計算を繋げるだけでよい

### 連鎖律の活用プロセス

```
【順伝播】入力 → 層1 → 層2 → 層3 → 出力 → 損失L
             ↓    ↓    ↓    ↓         ↑
           各層の出力を保存           誤差計算
...
| 場面 | 連鎖律の必要性 | 理由 |
|------|-------------|------|
| **誤差逆伝播** | **必須** | 合成関数の微分計算に不可欠 |
| **勾配降下法** | 必須 | 勾配計算に連鎖律を使用 |
| **順伝播** | 不要 | 単なる関数の合成計算 |
| **損失計算** | 不要 | 微分不要の計算 |
| **データ前処理** | 不要 | 微分と無関係 |
| **モデル評価** | 不要 | 精度計算等に微分不要 |
...
**効率性**：
- 順伝播と逆伝播の計算量がほぼ同等
- 全ての重みの勾配を $O(n)$ で計算可能（$n$ = パラメータ数）

**一般性**：
- どのような複雑なネットワーク構造でも適用可能
- CNN、RNN、Transformerなど全てで連鎖律を使用

**自動微分**：
- PyTorch、TensorFlowなどのフレームワークは連鎖律を自動で適用
- 計算グラフを構築し、自動的に勾配を計算
